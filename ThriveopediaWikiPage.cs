...
#pragma warning restore CA2213
    ...
    public override void _Ready()
    {
        ...
        if (pageSectionScene == null)
        {
            GD.PrintErr($"Failed to load wiki section scene for page {PageName}");
            return;
        }
    ...
    protected void AddSection(GameWiki.Page.Section content)
    {
        ...
        if (section == null)
        {
            GD.PrintErr($"Failed to instantiate wiki section for page {PageName}");
            return;
        }
    ...
    {
        var pageInstance = SceneLoadHelpers.LoadAndInstantiate<T>(scenePath,
            $"generating Thriveopedia wiki page {page.InternalName}");

        if (pageInstance == null)
        {
            GD.PrintErr($"Skipping Thriveopedia wiki page {page.InternalName} due to scene instantiate failure");
            return;
        }
        ...
        where T : ThriveopediaWikiPage
    {
        foreach (var page in pages)
        {
            ...
            if (page.NoticeSceneName != null)
            {
                ...
                var noticeInstance = SceneLoadHelpers.LoadAndInstantiate<Node>(noticePath,
                    $"generating notice scene for wiki page {page.InternalName}");

                if (noticeInstance == null)
                {
                    GD.PrintErr($"Skipping notice {page.NoticeSceneName} for page {page.InternalName}");
                }
                else
                {
