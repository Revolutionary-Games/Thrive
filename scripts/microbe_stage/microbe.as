...
class MicrobeComponent{
    ...
    void MicrobeComponent.load(storage){
    ...
    void MicrobeComponent.storage(storage){
    // Organelles
    ...
    class MicrobeSystem{
        ...
        this.microbes = {}
                ...
                compoundBag = CompoundBagComponent
                                                                                     ...
                                                                                     void MicrobeSystem.init(gameState){
                                                                                         LuaSystem.init(self, "MicrobeSystem", gameState)
                                                                                         this.entities.init(gameState.wrapper)
                                                                                         this.microbeCollisions.init(gameState.wrapper)
                                                                                         this.agentCollisions.init(gameState.wrapper)
                                                                                     this.bacteriaCollisions.init(gameState.wrapper)
                                                                                     }

                                                                                     void MicrobeSystem.shutdown(){
                                                                                     LuaSystem.shutdown(self)
                                                                                     this.entities.shutdown()
                                                                                     this.microbeCollisions.shutdown()
                                                                                         this.agentCollisions.shutdown()
                                                                                         this.bacteriaCollisions.shutdown()
                                                                                     }

                                                                                     void MicrobeSystem.update(renderTime, logicTime){
                                                                                                                                                                                             ...
                                                                                                                                                                                             for(_, collision in pairs(this.bacteriaCollisions.collisions())){
                                                                                                                                                                                                        local microbe_entity = Entity(collision.entityId1, this.gameState.wrapper)
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            if(microbe_entity.exists() and bacterium_entity.exists()){
                                                                                                                                                                                                                                  if(not (getComponent(bacterium_entity, Bacterium.COMPONENTS.bacterium) == null)){
                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                  void MicrobeSystem.checkEngulfment(engulferMicrobeEntity, engulfedMicrobeEntity){
    ...
    // The amount stored in the microbe's storage oraganelles
                                                                                                                                                                                                                                  void MicrobeSystem.getCompoundAmount(microbeEntity, compoundId){
    ...
    void MicrobeSystem.getSpeciesComponent(microbeEntity){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    ...
    // Sets the color of the microbe's membrane.
                                                                                                                                                                                                                                  void MicrobeSystem.setMembraneColour(microbeEntity, colour){
                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                  void MicrobeSystem.flashMembraneColour(microbeEntity, duration, colour){
                                                                                                                                                                                                                                      auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
                                                                                                                                                                                                                                      if(microbeComponent.flashDuration == null){
    ...
    void MicrobeSystem.storeCompound(microbeEntity, compoundId, amount, bandwidthLimited){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    auto storedAmount = amount + 0 // Why are we adding 0? Is this a type-casting thing?
    ...
    // Removes compounds from the microbe's storage organelles
                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                  void MicrobeSystem.takeCompound(microbeEntity, compoundId, maxAmount){
                                                                                                                                                                                                                                  auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                  // @param amount
                                                                                                                                                                                                                                  // The amount to eject
                                                                                                                                                                                                                                  void MicrobeSystem.ejectCompound(microbeEntity, compoundId, amount){
                                                                                                                                                                                                                                  auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
                                                                                                                                                                                                                                          auto membraneComponent = getComponent(microbeEntity, MembraneComponent)
...
auto amountToEject = MicrobeSystem.takeCompound(microbeEntity, compoundId, amount/10.0)
    ...
    void MicrobeSystem.respawnPlayer(){
    ...
    void MicrobeSystem.getOrganelleAt(microbeEntity, q, r){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    ...
    // Axial coordinates of the organelle's center
                ...
                // at (q,r)
                    void MicrobeSystem.removeOrganelle(microbeEntity, q, r){
                    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
                ...
                void MicrobeSystem.purgeCompounds(microbeEntity){
                    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
                ...
                for(_, compoundId in pairs(CompoundRegistry.getCompoundList())){
                           auto price = compoundBag.getPrice(compoundId)
                            if(price <= 0){
                                auto amountToEject = MicrobeSystem.getCompoundAmount(microbeEntity, compoundId)
                                if(amount > 0){ amountToEject = MicrobeSystem.takeCompound(microbeEntity, compoundId, amountToEject) }
                            if(amount > 0){ MicrobeSystem.ejectCompound(microbeEntity, compoundId, amountToEject) }
                            }
                            }
...
auto priceSum = 0
    ...
    auto amountToEject = compoundAmountToDump * (MicrobeSystem.getCompoundAmount(microbeEntity, compoundId) / price) / priceSum
    if(amount > 0){ amountToEject = MicrobeSystem.takeCompound(microbeEntity, compoundId, amountToEject) }
    if(amount > 0){ MicrobeSystem.ejectCompound(microbeEntity, compoundId, amountToEject) }
    }
    }
    ...
    void MicrobeSystem.removeEngulfedEffect(microbeEntity){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    ...
    void MicrobeSystem.addOrganelle(microbeEntity, q, r, rotation, organelle){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    auto membraneComponent = getComponent(microbeEntity, MembraneComponent)
    auto rigidBodyComponent = getComponent(microbeEntity, RigidBodyComponent)
    ...
    void MicrobeSystem.validPlacement(microbeEntity, organelle, q, r){
    auto touching = false;
    ...
    void MicrobeSystem.splitOrganelle(microbeEntity, organelle){
    auto q = organelle.position.q
    auto r = organelle.position.r
    ...
    auto data = {["name"]=organelle.name, ["q"]=q, ["r"]=r, ["rotation"]=i*60}
    ...
    void MicrobeSystem.toggleEngulfMode(microbeEntity){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    auto rigidBodyComponent = getComponent(microbeEntity, RigidBodyComponent)
    ...
    soundSourceComponent.stopSound("microbe-engulfment") // Possibly comment out. If version > 0.3.2 delete. //> We're way past 0.3.2, do we still need this?
                                                       ...
                                                       microbeComponent.movementFactor = microbeComponent.movementFactor / ENGULFING_MOVEMENT_DIVISION
                                                           ...
                                                           void MicrobeSystem.kill(microbeEntity){
                                                           auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
                                                           auto rigidBodyComponent = getComponent(microbeEntity, RigidBodyComponent)
                                                           auto soundSourceComponent = getComponent(microbeEntity, SoundSourceComponent)
                                                           ...
                                                           // Hacky but meh.
                                                           if(microbeComponent.dead){ return }
...
for(_, compoundId in pairs(CompoundRegistry.getCompoundList())){
           auto total = MicrobeSystem.getCompoundAmount(microbeEntity, compoundId)
               auto ejectedAmount = MicrobeSystem.takeCompound(microbeEntity, compoundId, total)
               ...
               for(_, organelle in pairs(microbeComponent.organelles)){
                          for(compoundName, amount in pairs(organelleTable[organelle.name].composition)){
                                                auto compoundId = CompoundRegistry.getCompoundId(compoundName)
                                                    if(compoundsToRelease[compoundId] == null)){
...
void MicrobeSystem.damage(microbeEntity, amount, damageType){
    assert(damageType !is null, "Damage type is null")
    ...
    auto rand = math.random(1, microbeComponent.maxHitpoints/MICROBE_HITPOINTS_PER_ORGANELLE)
    auto i = 1
    for(_, organelle in pairs(microbeComponent.organelles)){
...
void MicrobeSystem.atpDamage(microbeEntity){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)

    if(MicrobeSystem.getCompoundAmount(microbeEntity, CompoundRegistry.getCompoundId("atp")) < 1.0){
...
void MicrobeSystem.emitAgent(microbeEntity, compoundId, maxAmount){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    auto sceneNodeComponent = getComponent(microbeEntity, OgreSceneNodeComponent)
    auto soundSourceComponent = getComponent(microbeEntity, SoundSourceComponent)
    auto membraneComponent = getComponent(microbeEntity, MembraneComponent)
    ...
    if(numberOfAgentVacuoles == null or numberOfAgentVacuoles == 0){ return }
    ...
    if(MicrobeSystem.getCompoundAmount(microbeEntity, compoundId) > MINIMUM_AGENT_EMISSION_AMOUNT){
        ...
        auto membraneCoords = membraneComponent.getExternOrganellePos(organelleX, organelleY)
        ...
        direction.normalise()
        auto amountToEject = MicrobeSystem.takeCompound(microbeEntity, compoundId, maxAmount/10.0)
...
void MicrobeSystem.transferCompounds(fromEntity, toEntity){
    for(_, compoundID in pairs(CompoundRegistry.getCompoundList())){
...
void MicrobeSystem.createMicrobeEntity(name, aiControlled, speciesName, in_editor){
...
void MicrobeSystem.calculateStorageSpace(microbeEntity){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
...
void MicrobeSystem.updateCompoundAbsorber(microbeEntity){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    auto compoundAbsorberComponent = getComponent(microbeEntity, CompoundAbsorberComponent)
...
void MicrobeSystem.divide(microbeEntity){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    auto soundSourceComponent = getComponent(microbeEntity, SoundSourceComponent)
    auto membraneComponent = getComponent(microbeEntity, MembraneComponent)
    auto rigidBodyComponent = getComponent(microbeEntity, RigidBodyComponent)
    ...
    auto copyEntity = MicrobeSystem.createMicrobeEntity(null, true, microbeComponent.speciesName, false)
...
void MicrobeSystem.readyToReproduce(microbeEntity){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
...
void MicrobeSystem.updateMicrobe(microbeEntity, logicTime){
    auto microbeComponent = getComponent(microbeEntity, MicrobeComponent)
    auto membraneComponent = getComponent(microbeEntity, MembraneComponent)
    auto sceneNodeComponent = getComponent(microbeEntity, OgreSceneNodeComponent)
    auto compoundAbsorberComponent = getComponent(microbeEntity, CompoundAbsorberComponent)
    ...
    if(not microbeComponent.dead){
        ...
        if(microbeComponent.flashDuration !is null and microbeComponent.flashColour !is null){
            microbeComponent.flashDuration = microbeComponent.flashDuration - logicTime
            
            auto entity = membraneComponent.entity
            // How frequent it flashes, would be nice to update the flash void to have this variable{
            if(math.fmod(microbeComponent.flashDuration, 600) < 300){
            ...
            if(microbeComponent.flashDuration <= 0){
                microbeComponent.flashDuration = null				
                entity.setMaterial(sceneNodeComponent.meshName)
            }
        }
...
void MicrobeSystem.initializeMicrobe(microbeEntity, in_editor){
    ...
    for(key, ctype in pairs(MICROBE_COMPONENTS)){
        ...
        assert(component !is null, "Can't create microbe from this entity, it's missing " .. key)
    }
    ...
    if(not microbeComponent.initialized){
        // TODO: cache for performance
        auto compoundShape = CompoundShape.castFrom(rigidBodyComponent.properties.shape)
