    ...
    double maxAmount, float lifeTime)
{
    ...
    if(compoundBag.getCompoundAmount(compoundId) > MINIMUM_AGENT_EMISSION_AMOUNT)
        ...
        if (amountToEject >= MINIMUM_AGENT_EMISSION_AMOUNT)
        {
            ...
            //createAgentCloud(world, cellPosition._Position+Float3(xnew*ejectionDistance,0,ynew*ejectionDistance),direction, lifeTime, microbeComponent.speciesName);
...
void kill(CellStageWorld@ world, ObjectID microbeEntity)
{
    ...
    for(uint i = 0; i < storageTypes.length(); ++i){
        ...
        while(_amount > 0){
            ...
            //createAgentCloud(world, position._Position, direction, 2000, microbeComponent.speciesName);
            ++createdAgents;
