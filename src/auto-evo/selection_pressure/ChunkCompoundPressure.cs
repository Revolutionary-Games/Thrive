...
public class ChunkCompoundPressure : SelectionPressure
{
    ...
    public override float Score(Species species, Patch patch, SimulationCache cache)
    {
        ...
        var chemoreceptorScore = cache.GetChemoreceptorChunkScore(species, chunk, compound);
        ...
        if (compoundOut != atp)
        {
            var compoundOutGenerated =
                cache.GetCompoundGeneratedFrom(compound, compoundOut, species, patch.Biome);
            compoundATP = cache.GetCompoundConversionScoreForSpecies(compoundOut, atp, species) *
                compoundOutGenerated;
        }
        else
        {
            compoundATP = cache.GetCompoundGeneratedFrom(compound, atp, species, patch.Biome);
        }
