...
public class PredationEffectivenessPressure : SelectionPressure
{
    ...
    public PredationEffectivenessPressure(Species prey, float weight) :
        base(weight, [
            ...
            new UpgradeOrganelle(organelle => organelle.HasLysosomeComponent, new LysosomeUpgrades(SimulationParameters.Instance.GetEnzyme(Constants.CHITINASE_ENZYME))),
            new UpgradeOrganelle(organelle => organelle.HasLysosomeComponent, new LysosomeUpgrades(SimulationParameters.Instance.GetEnzyme(Constants.CELLULASE_ENZYME))),
