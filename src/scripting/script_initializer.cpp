 ...
 */
void
    readLuaCallstack(lua_State* L, lua_Debug& d, std::stringstream& traceback);
    ...
    readLuaCallstack(lua_State* L, lua_Debug& d, std::stringstream& traceback)
{
    ...
    for(
        ...
        int stacklevel = 1; lua_getstack(L, stacklevel, &d); stacklevel++) {
        if(lua_getinfo(L, "Sln", &d) == 0) {

            traceback << "    "
                      << "error getting stack frame" << std::endl;
            continue;
        }
    ...
    thriveLuaPanic(lua_State* L)
{
    ...
    // TODO: check if we should push this string and is throwing from here a
    // good idea looks like throwing from here is a good idea
...
std::string
    thriveLuaOnError(sol::this_state lua, std::string err);
    ...
    thriveLuaOnError(sol::this_state lua, std::string err)
{
    ...
    //    "An unexpected error occurred and forced the lua state to call
    //    atpanic";
    // lua_pop(L, 1);

    if(err.empty())
        err = "An unexpected error occurred and forced the lua state to call "
              "onerror";
...
void
    bindClassesToLua(sol::state& lua);

// Forward declare some binding functions
static void
    listboxItemBindings(sol::state& lua);
static void
    itemEntryluaBindings(sol::state& lua);
static void
    ogreLuaBindings(sol::state& lua);

void
    thrive::initializeLua(sol::state& lua)
{
    ...
    lua.open_libraries(sol::lib::base, sol::lib::jit,
        ...
        sol::lib::debug, sol::lib::coroutine, sol::lib::string, sol::lib::math,
        sol::lib::table, sol::lib::package, sol::lib::io, sol::lib::os
...
static void
    ListboxItem_setColour(CEGUI::ListboxTextItem& self,
        float r,
        float g,
        float b)
{
...
static void
    ListboxItem_setText(CEGUI::ListboxTextItem& self, const std::string& text)
{
    self.setText(text);
}
    ...
    listboxItemBindings(sol::state& lua)
{
    ...
    lua.new_usertype<CEGUI::ListboxTextItem>("ListboxItem",
        ...
        "setTextColours", &ListboxItem_setColour, "setText",
        &ListboxItem_setText);
}

static void
    ItemEntry_setText(CEGUI::ItemEntry& self, const std::string& text)
{
    self.setText(text);
}

static bool
    ItemEntry_isSelected(CEGUI::ItemEntry& self)
{
    return self.isSelected();
}

static void
    ItemEntry_select(CEGUI::ItemEntry& self)
{
    self.select();
}

static void
    ItemEntry_deselect(CEGUI::ItemEntry& self)
{
    self.deselect();
}

static void
    ItemEntry_setSelectable(CEGUI::ItemEntry& self, bool setting)
{
    ...
    itemEntryluaBindings(sol::state& lua)
{
    ...
    lua.new_usertype<CEGUI::ItemEntry>("ItemEntry",
        ...
        "isSelected", &ItemEntry_isSelected, "select", &ItemEntry_select,
        "deselect", &ItemEntry_deselect, "setSelectable",
        &ItemEntry_setSelectable, "setText", &ItemEntry_setText);
}

static void
    axisAlignedBoxBindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::AxisAlignedBox>("BoxShape",
        ...
        sol::constructors<sol::types<>,
            sol::types<Ogre::AxisAlignedBox::Extent>,
            sol::types<const Ogre::Vector3&, const Ogre::Vector3&>,
            sol::types<Ogre::Real, Ogre::Real, Ogre::Real, Ogre::Real,
                Ogre::Real, Ogre::Real>>(),

        sol::meta_function::equal_to, &Ogre::AxisAlignedBox::operator==,

        "Extent", sol::var(lua.create_table_with("EXTENT_NULL",
            Ogre::AxisAlignedBox::EXTENT_NULL, "EXTENT_FINITE",
            Ogre::AxisAlignedBox::EXTENT_FINITE, "EXTENT_INFINITE",
            Ogre::AxisAlignedBox::EXTENT_INFINITE)),

        "CornerEnum",
        sol::var(lua.create_table_with("FAR_LEFT_BOTTOM",
            Ogre::AxisAlignedBox::FAR_LEFT_BOTTOM, "FAR_LEFT_TOP",
            Ogre::AxisAlignedBox::FAR_LEFT_TOP, "FAR_RIGHT_TOP",
            Ogre::AxisAlignedBox::FAR_RIGHT_TOP, "FAR_RIGHT_BOTTOM",
            Ogre::AxisAlignedBox::FAR_RIGHT_BOTTOM, "NEAR_RIGHT_BOTTOM",
            Ogre::AxisAlignedBox::NEAR_RIGHT_BOTTOM, "NEAR_LEFT_BOTTOM",
            Ogre::AxisAlignedBox::NEAR_LEFT_BOTTOM, "NEAR_LEFT_TOP",
            Ogre::AxisAlignedBox::NEAR_LEFT_TOP, "NEAR_RIGHT_TOP",
        ...
        "setMinimum",
        sol::overload(static_cast<void (AxisAlignedBox::*)(const Vector3&)>(
                          &AxisAlignedBox::setMinimum),
            static_cast<void (AxisAlignedBox::*)(Real, Real, Real)>(
                &AxisAlignedBox::setMinimum)),

        "setMinimumX", &AxisAlignedBox::setMinimumX, "setMinimumY",
        &AxisAlignedBox::setMinimumY, "setMinimumZ",
        &AxisAlignedBox::setMinimumZ,
        ...
        "setMaximumX", &AxisAlignedBox::setMaximumX, "setMaximumY",
        &AxisAlignedBox::setMaximumY, "setMaximumZ",
        &AxisAlignedBox::setMaximumZ, "setExtents",
        sol::overload(static_cast<void (AxisAlignedBox::*)(const Vector3&,
        ...
        "getCorner", &AxisAlignedBox::getCorner, "merge",
        sol::overload(
        ...
        "setNull", &AxisAlignedBox::setNull, "isNull", &AxisAlignedBox::isNull,
        "isFinite", &AxisAlignedBox::isFinite, "setInfinite",
        &AxisAlignedBox::setInfinite, "isInfinite", &AxisAlignedBox::isInfinite,
        "intersects",
        sol::overload(
        ...
        "intersection", &AxisAlignedBox::intersection, "volume",
        &AxisAlignedBox::volume, "scale", &AxisAlignedBox::scale,

        "getCenter", &AxisAlignedBox::getCenter, "getSize",
        &AxisAlignedBox::getSize, "getHalfSize", &AxisAlignedBox::getHalfSize,
        "contains",
        sol::overload(
            static_cast<bool (AxisAlignedBox::*)(const Vector3&) const>(
...
static void
    colourValueBindings(sol::state& lua)
{
    ...
    lua.new_usertype<ColourValue>("ColourValue",
        ...
        "r", &ColourValue::r, "g", &ColourValue::g, "b", &ColourValue::b, "a",
        &ColourValue::a);
}

static void
    degreeBindings(sol::state& lua)
{

    lua.new_usertype<Ogre::Degree>("Degree",

        sol::constructors<sol::types<Ogre::Real>,
            sol::types<const Ogre::Radian&>>(),
        ...
        sol::overload(static_cast<Ogre::Degree (Ogre::Degree::*)(
...
static void
    SubEntity_setColour(Ogre::SubEntity& self, const Ogre::ColourValue& colour)
{
...
static void
    Entity_setColour(Ogre::Entity& self, const Ogre::ColourValue& colour)
{
    auto material = thrive::getColourMaterial(colour);
    self.setMaterial(material);
}
...
static void
    SubEntity_setMaterial(Ogre::SubEntity& self, const Ogre::String& name)
{
    Ogre::MaterialManager& manager = Ogre::MaterialManager::getSingleton();
    Ogre::MaterialPtr material = manager.getByName(name);
    self.setMaterial(material);
}

static void
    Entity_setMaterial(Ogre::Entity& self, const Ogre::String& name)
{
    Ogre::MaterialManager& manager = Ogre::MaterialManager::getSingleton();
    Ogre::MaterialPtr material = manager.getByName(name);
    self.setMaterial(material);
...
static void
    SubEntity_tintColour(Ogre::SubEntity& self,
        ...
        const Ogre::ColourValue& colour)
{
    ...
    ptus->setColourOperationEx(
        Ogre::LBX_MODULATE, Ogre::LBS_MANUAL, Ogre::LBS_TEXTURE, colour);
    self.setMaterial(materialPtr);
}

static void
    Entity_tintColour(Ogre::Entity& self,
        const Ogre::String& materialName,
        const Ogre::ColourValue& colour)
{
    Ogre::MaterialPtr baseMaterial =
        Ogre::MaterialManager::getSingleton().getByName(materialName);
    Ogre::MaterialPtr materialPtr = baseMaterial->clone(
        materialName + std::to_string(static_cast<int>(colour.r * 256)) +
        std::to_string(static_cast<int>(colour.g * 256)) +
        std::to_string(static_cast<int>(colour.b * 256)));
    materialPtr->compile();
    Ogre::TextureUnitState* ptus =
        materialPtr->getTechnique(0)->getPass(0)->getTextureUnitState(0);
    ptus->setAlphaOperation(
        Ogre::LBX_MODULATE, Ogre::LBS_MANUAL, Ogre::LBS_TEXTURE, colour.a);
    ptus->setColourOperationEx(
        Ogre::LBX_MODULATE, Ogre::LBS_MANUAL, Ogre::LBS_TEXTURE, colour);
    self.setMaterial(materialPtr);
}
...
static void
    Entity_cloneMaterial(Ogre::Entity& self, const Ogre::String& materialName)
{
    Ogre::MaterialPtr baseMaterial =
        Ogre::MaterialManager::getSingleton().getByName(materialName);
    Ogre::MaterialPtr materialPtr =
        baseMaterial->clone(materialName + std::to_string(clonedIndex));
    materialPtr->compile();
...
static void
    Entity_setMaterialColour(Ogre::Entity& self,
        // const String& materialName,
        const Ogre::ColourValue& colour)
{
    // Ogre::MaterialPtr baseMaterial =
    // Ogre::MaterialManager::getSingleton().getByName(materialName);
    // Ogre::MaterialPtr materialPtr = baseMaterial->clone(materialName +
    // std::to_string(clonedIndex));  materialPtr->compile();
    // self.setMaterialName(materialName + std::to_string(clonedIndex));
    // clonedIndex++;

    Ogre::SubMesh* sub = self.getMesh()->getSubMesh(0);
    Ogre::MaterialPtr materialPtr =
        Ogre::MaterialManager::getSingleton().getByName(sub->getMaterialName());
    Ogre::TextureUnitState* ptus =
        materialPtr->getTechnique(0)->getPass(0)->getTextureUnitState(0);
    ptus->setColourOperationEx(
        Ogre::LBX_MODULATE, Ogre::LBS_MANUAL, Ogre::LBS_TEXTURE, colour);
...
static void
    ogreEntityBindings(sol::state& lua)
{

    lua.new_usertype<Ogre::SubEntity>("OgreSubEntity",

        "setColour", &SubEntity_setColour, "setMaterial",
        &SubEntity_setMaterial, "tintColour", &SubEntity_tintColour);

    lua.new_usertype<Ogre::Entity>("OgreEntity",

        "getSubEntity",
        static_cast<Ogre::SubEntity* (Ogre::Entity::*)(const Ogre::String&)>(
            &Ogre::Entity::getSubEntity),
        "getNumSubEntities", &Ogre::Entity::getNumSubEntities, "setColour",
        &Entity_setColour, "setMaterial", &Entity_setMaterial, "cloneMaterial",
        &Entity_cloneMaterial, "setMaterialColour", &Entity_setMaterialColour,
    ...
    matrix3Bindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::Matrix3>("Matrix3",
        ...
        sol::constructors<sol::types<>,
            sol::types<Ogre::Real, Ogre::Real, Ogre::Real, Ogre::Real,
                Ogre::Real, Ogre::Real, Ogre::Real, Ogre::Real, Ogre::Real>>(),
        ...
        sol::meta_function::subtraction,
        static_cast<Ogre::Matrix3 (Ogre::Matrix3::*)(const Ogre::Matrix3&)
                const>(&Ogre::Matrix3::operator-),

        sol::meta_function::multiplication,
        sol::overload(
            static_cast<Ogre::Matrix3 (Ogre::Matrix3::*)(const Ogre::Matrix3&)
                    const>(&Ogre::Matrix3::operator*),
            static_cast<Ogre::Matrix3 (Ogre::Matrix3::*)(const Ogre::Real)
                    const>(&Ogre::Matrix3::operator*),
            static_cast<Ogre::Vector3 (Ogre::Matrix3::*)(const Ogre::Vector3&)
                    const>(&Ogre::Matrix3::operator*)),

        "GetColumn", &Matrix3::GetColumn, "SetColumn", &Matrix3::SetColumn,
        "FromAxes", &Matrix3::FromAxes, "Transpose", &Matrix3::Transpose,
        "Inverse",
        static_cast<bool (Matrix3::*)(Matrix3&, Real) const>(&Matrix3::Inverse),

        "Determinant", &Matrix3::Determinant, "SingularValueDecomposition",
        &Matrix3::SingularValueDecomposition, "SingularValueComposition",
        &Matrix3::SingularValueComposition, "Orthonormalize",
        &Matrix3::Orthonormalize, "QDUDecomposition",
        &Matrix3::QDUDecomposition, "SpectralNorm", &Matrix3::SpectralNorm,
        "ToAngleAxis",
        static_cast<void (Matrix3::*)(Vector3&, Radian&) const>(
            &Matrix3::ToAngleAxis),

        "FromAngleAxis", &Matrix3::FromAngleAxis, "ToEulerAnglesXYZ",
        &Matrix3::ToEulerAnglesXYZ, "ToEulerAnglesXZY",
        &Matrix3::ToEulerAnglesXZY, "ToEulerAnglesYXZ",
        &Matrix3::ToEulerAnglesYXZ, "ToEulerAnglesYZX",
        &Matrix3::ToEulerAnglesYZX, "ToEulerAnglesZXY",
        &Matrix3::ToEulerAnglesZXY, "ToEulerAnglesZYX",
        &Matrix3::ToEulerAnglesZYX, "FromEulerAnglesXYZ",
        &Matrix3::FromEulerAnglesXYZ, "FromEulerAnglesXZY",
        &Matrix3::FromEulerAnglesXZY, "FromEulerAnglesYXZ",
        &Matrix3::FromEulerAnglesYXZ, "FromEulerAnglesYZX",
        &Matrix3::FromEulerAnglesYZX, "FromEulerAnglesZXY",
        &Matrix3::FromEulerAnglesZXY, "FromEulerAnglesZYX",
        &Matrix3::FromEulerAnglesZYX, "hasScale", &Matrix3::hasScale);
}

static void
    planeBindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::Plane>("Plane",
        ...
        sol::constructors<sol::types<>, sol::types<const Vector3&, Real>,
            sol::types<Real, Real, Real, Real>,
            sol::types<const Vector3&, const Vector3&>,
            sol::types<const Vector3&, const Vector3&, const Vector3&>>(),

        // sol::meta_function::equal_to, &Ogre::Plane::operator==,

        "Side",
        sol::var(lua.create_table_with("NO_SIDE", Plane::NO_SIDE,
            "POSITIVE_SIDE", Plane::POSITIVE_SIDE, "NEGATIVE_SIDE",
            Plane::NEGATIVE_SIDE, "BOTH_SIDE", Plane::BOTH_SIDE)),

        "getSide",
        sol::overload(static_cast<Plane::Side (Plane::*)(const Vector3&) const>(
                          &Plane::getSide),
            static_cast<Plane::Side (Plane::*)(const AxisAlignedBox&) const>(
                &Plane::getSide),
            static_cast<Plane::Side (Plane::*)(const Vector3&, const Vector3&)
                    const>(&Plane::getSide)),
        ...
        "redefine",
        sol::overload(
            static_cast<void (Plane::*)(const Vector3&, const Vector3&)>(
                &Plane::redefine),
            static_cast<void (Plane::*)(const Vector3&, const Vector3&,
                const Vector3&)>(&Plane::redefine)),

        "projectVector", &Plane::projectVector, "normalise", &Plane::normalise,
        "normal", &Plane::normal, "d", &Plane::d);
}

static void
    quaternionBindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::Quaternion>("Quaternion",
        ...
        sol::constructors<sol::types<>, sol::types<const Matrix3&>,
            sol::types<Real, Real, Real, Real>,
            sol::types<const Radian&, const Vector3&>,
            sol::types<const Vector3&, const Vector3&, const Vector3&>>(),
        ...
        sol::meta_function::subtraction,
        static_cast<Ogre::Quaternion (Ogre::Quaternion::*)(
        ...
        sol::overload(
            static_cast<Ogre::Quaternion (Ogre::Quaternion::*)(
                const Ogre::Quaternion&) const>(&Ogre::Quaternion::operator*),
            static_cast<Ogre::Quaternion (Ogre::Quaternion::*)(const Ogre::Real)
                    const>(&Ogre::Quaternion::operator*),
            static_cast<Ogre::Vector3 (Ogre::Quaternion::*)(
                const Ogre::Vector3&) const>(&Ogre::Quaternion::operator*)),

        "FromRotationMatrix", &Quaternion::FromRotationMatrix,
        "ToRotationMatrix", &Quaternion::ToRotationMatrix, "FromAngleAxis",
        &Quaternion::FromAngleAxis, "ToAngleAxis",
        static_cast<void (Quaternion::*)(Radian&, Vector3&) const>(
            &Quaternion::ToAngleAxis),
        "FromAxes",
        static_cast<void (Quaternion::*)(const Vector3&, const Vector3&,
            const Vector3&)>(&Quaternion::FromAxes),
        "ToAxes",
        static_cast<void (Quaternion::*)(Vector3&, Vector3&, Vector3&) const>(
            &Quaternion::ToAxes),
        "xAxis", &Quaternion::xAxis, "yAxis", &Quaternion::yAxis, "zAxis",
        &Quaternion::zAxis, "Dot", &Quaternion::Dot, "Norm", &Quaternion::Norm,
        "normalise", &Quaternion::normalise, "Inverse", &Quaternion::Inverse,
        "UnitInverse", &Quaternion::UnitInverse, "Exp", &Quaternion::Exp, "Log",
        &Quaternion::Log, "getRoll", &Quaternion::getRoll, "getPitch",
        &Quaternion::getPitch, "getYaw", &Quaternion::getYaw, "equals",
        &Quaternion::equals, "isNaN", &Quaternion::isNaN);
}


static void
    radianBindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::Radian>("Radian",
        ...
        sol::meta_function::multiplication,
        sol::overload(static_cast<Ogre::Radian (Ogre::Radian::*)(
    ...
    rayBindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::Ray>("Ray",
        ...
        sol::meta_function::multiplication,
        static_cast<Ogre::Vector3 (Ogre::Ray::*)(Ogre::Real) const>(
            &Ogre::Ray::operator*),

        "setOrigin", &Ray::setOrigin, "getOrigin", &Ray::getOrigin,
        "setDirection", &Ray::setDirection, "getDirection", &Ray::getDirection,
        "getPoint", &Ray::getPoint,
        // returns a tuple now
        "intersects",
        static_cast<std::pair<bool, Ogre::Real> (Ogre::Ray::*)(
            const Ogre::Plane&) const>(&Ray::intersects));
}


static void
    sceneManagerBindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::SceneManager>("SceneManager",
        ...
        "PrefabType",
        sol::var(
            lua.create_table_with("PT_PLANE", SceneManager::PT_PLANE, "PT_CUBE",
                SceneManager::PT_CUBE, "PT_SPHERE", SceneManager::PT_SPHERE)),

        "setAmbientLight", &SceneManager::setAmbientLight);
}

static void
    sphereBindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::Sphere>("Sphere",
        ...
        "getRadius", &Sphere::getRadius, "setRadius", &Sphere::setRadius,
        "getCenter", &Sphere::getCenter, "setCenter", &Sphere::setCenter,

        "intersects",
        sol::overload(static_cast<bool (Sphere::*)(const Sphere&) const>(
                          &Sphere::intersects),
            static_cast<bool (Sphere::*)(const AxisAlignedBox&) const>(
                &Sphere::intersects),
            static_cast<bool (Sphere::*)(const Plane&) const>(
                &Sphere::intersects),
            static_cast<bool (Sphere::*)(const Vector3&) const>(
                &Sphere::intersects)),
    ...
    vector3Bindings(sol::state& lua)
{
    ...
    lua.new_usertype<Ogre::Vector3>("Vector3",
        ...
        sol::constructors<sol::types<>,
            sol::types<const Real, const Real, const Real>>(),
        ...
        sol::meta_function::addition,
        static_cast<Ogre::Vector3 (Ogre::Vector3::*)(const Ogre::Vector3&)
        ...
        "length", &Vector3::length, "squaredLength", &Vector3::squaredLength,
        "distance", &Vector3::distance, "squaredDistance",
        &Vector3::squaredDistance, "dotProduct", &Vector3::dotProduct,
        "absDotProduct", &Vector3::absDotProduct, "normalise",
        &Vector3::normalise, "crossProduct", &Vector3::crossProduct, "midPoint",
        &Vector3::midPoint, "makeFloor", &Vector3::makeFloor, "makeCeil",
        &Vector3::makeCeil, "perpendicular", &Vector3::perpendicular,
        "randomDeviant", &Vector3::randomDeviant, "angleBetween",
        &Vector3::angleBetween, "getRotationTo", &Vector3::getRotationTo,
        "isZeroLength", &Vector3::isZeroLength, "normalisedCopy",
        &Vector3::normalisedCopy, "reflect", &Vector3::reflect,
        "positionEquals", &Vector3::positionEquals, "positionCloses",
        &Vector3::positionCloses, "directionEquals", &Vector3::directionEquals,
        "isNaN", &Vector3::isNaN, "primaryAxis", &Vector3::primaryAxis);
}
    ...
    ogreLuaBindings(sol::state& lua)
{
    ...
    thrive::RenderSystem::luaBindings(
        lua); // Fully qualified because of Ogre::RenderSystem
