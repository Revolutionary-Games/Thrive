...
// TODO: optimize, template, and add support for a lower-level leaf
// datastructure
...
struct RollingGrid::Implementation {
    ...
    boost::multi_array<int, 2>
        m_data; // row-major (where rows are of constant x)
    typedef boost::multi_array<int, 2>::index Index;

    Implementation(int width, int height, int resolution) :
        m_width(width), m_height(height), m_resolution(resolution), m_x(0),
        m_y(0), m_rows(height / resolution), m_cols(width / resolution)
    {
    ...
    // Note -- they denote the row above and column left of the wraparound on
    // the /grid/
        ...
        operator()(long xin, long yin)
    {
        ...
        // essentially, we temp-move the grid to (xin, yin) and read at
        // (m_wrapr, m_wrapy)
...
void
    RollingGrid::luaBindings(sol::state& lua)
{
    lua.new_usertype<RollingGrid>("RollingGrid",

        sol::constructors<sol::types<int, int, int>>(), "move",
        &RollingGrid::move, "get", &RollingGrid::get, "set", &RollingGrid::set);
}
