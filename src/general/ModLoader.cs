...
public class ModLoader : Control
{
	...
	public override void _Ready()
	{
		 ...
		 DirectoryInfo modFolder = System.IO.Directory.CreateDirectory($"{System.IO.Directory.GetCurrentDirectory()}\\mods");
		 foreach(DirectoryInfo currentMod in modFolder.EnumerateDirectories()){
			if(!System.IO.File.Exists(currentMod.FullName + "/mod_info.json")) continue;
			var currentModInfo = JsonConvert.DeserializeObject<ModInfo>(ReadJSONFile(currentMod.FullName + "/mod_info.json"));
...
//  }
	...
	private void OnMoveToLoadPressed()
	{
		...
		if(unloadedItemList.GetSelectedItems().Length <= 0) return;
	...
	{
		GUICommon.Instance.PlayButtonPressSound();
		if(loadedItemList.GetSelectedItems().Length <= 0) return;
	...
	{
		GUICommon.Instance.PlayButtonPressSound();
		if(loadedModList.Count <= 0) return;
		foreach(ModInfo currentMod in loadedModList){
			if(currentMod.Dll != null || currentMod.Dll != ""){
				if(!System.IO.File.Exists(currentMod.Location + "/" + currentMod.Dll)) continue;
				Assembly.LoadFile(currentMod.Location + "/" + currentMod.Dll);
			}
			if(!System.IO.File.Exists(currentMod.Location + "/mod.pck")) continue;
