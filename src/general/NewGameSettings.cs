...
#pragma warning restore CA2213
    ...
    public override void _Ready()
    {
        ...
        osmoregulationMultiplier.MinValue = Constants.MIN_ENERGYCOST_MULTIPLIER;
        osmoregulationMultiplier.MaxValue = Constants.MAX_ENERGYCOST_MULTIPLIER;
    ...
    public void OpenFromDescendScreen(GameProperties currentGame)
    {
        ...
        osmoregulationMultiplier.Value = difficulty.EnergyCostMultiplier;
    ...
    private WorldGenerationSettings GetGameSettings()
    {
        ...
        if (difficulty.InternalName == custom.InternalName)
        {
            var customDifficulty = new CustomDifficulty
            {
                ...
                EnergyCostMultiplier = (float)osmoregulationMultiplier.Value,
    ...
    private void OnDifficultyPresetSelected(int index)
    {
        ...
        osmoregulationMultiplier.Value = preset.EnergyCostMultiplier;
    ...
    private void UpdateSelectedDifficultyPresetControl()
    {
        foreach (var preset in difficultyPresets)
        {
            ...
            if (Math.Abs((float)osmoregulationMultiplier.Value - preset.EnergyCostMultiplier) > MathUtils.EPSILON)
                continue;
