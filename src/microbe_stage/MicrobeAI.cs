...
/// <summary>
///   AI for a single Microbe. This is a separate class to contain all the AI
///   status variables as well as make the Microbe.cs file cleaner as this AI
///   has a lot of code.
/// </summary>
public
class MicrobeAI {
private
    readonly Compound atp;

    // ReSharper disable once NotAccessedField.Local
private
    readonly Compound iron;
private
    readonly Compound oxytoxy;

    [JsonProperty] private Microbe microbe;

    [JsonProperty] private int boredom;

    // ReSharper disable once CollectionNeverQueried.Local
    [JsonIgnore] private List<FloatingChunk> chunkList =
        new List<FloatingChunk>();

    [JsonProperty] private bool hasTargetPosition;

    [JsonProperty] private LifeState lifeState = LifeState.NEUTRAL_STATE;

    [JsonProperty] private bool moveFocused;

    [JsonProperty] private float movementRadius = 2000;

    [JsonProperty] private bool moveThisHunt = true;

    // All of the game entities stored here are probable places where disposed
    // objects come from so they are ignored for now
    [JsonIgnore] private Microbe predator;

    // Prey and predator lists
    [JsonIgnore] private List<Microbe> predatoryMicrobes = new List<Microbe>();

    [JsonProperty] private float previousAngle;

    [JsonIgnore] private Microbe prey;

    [JsonIgnore] private Microbe previousPrey;

    [JsonIgnore] private List<Microbe> preyMicrobes = new List<Microbe>();

    [JsonIgnore] private bool preyPegged;

    [JsonIgnore] private FloatingChunk targetChunk;

    [JsonIgnore] private FloatingChunk previousChunk;

    [JsonProperty] private Vector3 targetPosition = new Vector3(0, 0, 0);

    /// <summary>
    ///   TODO: change to be the elapsed time instead of AI update count
    /// </summary>
    [JsonProperty] private float ticksSinceLastToggle = 600;

public
    MicrobeAI(Microbe microbe)
    {
        this.microbe = microbe ? ? throw new ArgumentException(
                                       "no microbe given", nameof(microbe));
...
private
    float SpeciesFear = > microbe.Species.Fear;

private
    float SpeciesActivity = > microbe.Species.Activity;

private
    float SpeciesFocus = > microbe.Species.Focus;

private
    float SpeciesOpportunism = > microbe.Species.Opportunism;

public
    void
        Think(float delta, Random random, MicrobeAICommonData data)
    {
        _ = delta;
        ...
        // 30 seconds about
        if(boredom ==
            (int)random.Next(SpeciesFocus * 2, 1000.0f + SpeciesFocus * 2)) {
            ...
            if(targetChunk != null &&
                RollReverseCheck(SpeciesFocus, 200, random)) {
        ...
        case LifeState.PLANTLIKE_STATE:
            // This ai would ideally just sit there, until it sees a nice
            // opportunity pop-up unlike neutral, which wanders randomly (has a
            // gather chance) until something interesting pops up
            break;
        case LifeState.NEUTRAL_STATE: {
            // Before these would run every time, now they just run for the
...
private
    static bool
        RollCheck(float ourStat, float dc, Random random)
    {
...
private
    static bool
        RollReverseCheck(float ourStat, float dc, Random random)
    {
...
private
    void
        DoReflexes()
    {
        // For times when its best to tell the microbe directly what to do (Life
        // threatening, attaching to things etc);
        /* Check if we are willing to run, and there is a predator nearby, if
           so, flee for your life If it was ran in evaluate environment, it
           would only work if the microbe was in the neutral state. because we
           may need more of these very specific things in the future for things
           like latching onto rocks */
        // If you are predating and not being engulfed, don't run away until you
        // switch state (keeps predators chasing you even when their predators
        // are nearby) Its not a good survival strategy but it makes the game
        // more fun.
        if(predator != null && (lifeState != LifeState.PREDATING_STATE ||
                                   microbe.IsBeingEngulfed)) {
            try {
                if(!predator.Dead) {
                    if((microbe.Translation - predator.Translation)
                            .LengthSquared() <=
...
private
    FloatingChunk
        GetNearestChunkItem(List<FloatingChunk> allChunks)
    {
        ...
        foreach(var chunk in allChunks) {
            ...
            if((SpeciesOpportunism == Constants.MAX_SPECIES_OPPORTUNISM) ||
                ((microbe.EngulfSize *
                     (SpeciesOpportunism / Constants.OPPORTUNISM_DIVISOR)) >
                    chunk.Size)) {
                chunkList.Add(chunk);
...
private
    Microbe
        GetNearestPreyItem(List<Microbe> allMicrobes)
    {
        Microbe chosenPrey = null;

        // Use the agent amounts so a small cell with a lot of toxins has the
        // courage to attack.
        ...
        foreach(var otherMicrobe in allMicrobes) {
            ...
            if(otherMicrobe.Species != microbe.Species && !otherMicrobe.Dead) {
                if((SpeciesAggression == Constants.MAX_SPECIES_AGRESSION) ||
                    ((((microbe.AgentVacuoleCount + microbe.EngulfSize) *
                          1.0f) *
        ...
        // It might be interesting to prioritize weakened prey (Maybe add a
        // variable for opportunisticness to each species?)
        return chosenPrey;
    }
...
private
    void
        GetNearestPredatorItem(List<Microbe> allMicrobes)
    {
        // Retrive the nearest predator
        // For our desires lets just say all microbes bigger are potential
        // predators and later extend this to include those with toxins and
        // pilus
        Vector3 testPosition = new Vector3(0, 0, 0);
        bool setPosition = true;
        ...
        foreach(var otherMicrobe in allMicrobes) {
            ...
            if(otherMicrobe.Species != microbe.Species && !otherMicrobe.Dead) {
                if((SpeciesFear == Constants.MAX_SPECIES_FEAR) ||
                    ((((microbe.AgentVacuoleCount + otherMicrobe.EngulfSize) *
                          1.0f) *
...
private
    void
        DealWithPrey(List<Microbe> allMicrobes, Random random)
    {
        ...
        // Allows for emergence of Predatory Plants (Like a single cleed version
        // of a venus fly trap) Creatures with lethargicness of 400 will not
        // actually chase prey, just lie in wait
        ...
        // This is probabbly not working. This is almost certainly not working
        // in the Godot version
        if(prey.Dead) {
            hasTargetPosition = false;
            ...
            if((microbe.Translation - targetPosition).LengthSquared() <=
                    300 + microbe.EngulfSize * 3.0f &&
        ...
        // Shoot toxins if able There should be AI that prefers shooting over
        // engulfing, etc, not sure how to model that without a million and one
        // variables perhaps its a mix? Maybe a creature with a focus less then
        // a certain amount simply never attacks that way?  Maybe a creature
        // with a specific focus, only ever shoots and never engulfs? Maybe
        // their lethargicness impacts that? I just dont want each enemy to feel
        // the same you know.  For now creatures with a focus under 100 will
        // never shoot.
        if(SpeciesFocus >= 100.0f) {
            if(microbe.Hitpoints > 0 && microbe.AgentVacuoleCount > 0 &&
                (microbe.Translation - targetPosition).LengthSquared() <=
                    SpeciesFocus * 10.0f) {
                if(microbe.Compounds.GetCompoundAmount(oxytoxy) >=
...
private
    void
        DealWithChunks(FloatingChunk chunk, List<FloatingChunk> allChunks)
    {
        // Tick the engulf tick
        ticksSinceLastToggle += 1;
...
private
    void
        DealWithPredators(Random random)
    {
...
private
    void
        PreyFlee(Random random)
    {
        // If focused you can run away more specifically, if not you freak out
        // and scatter
        if(predator == null || !RollCheck(SpeciesFocus, 500.0f, random)) {
            ...
            targetPosition = new Vector3(Mathf.Cos(randAngle) * randDist, 0,
                Mathf.Sin(randAngle) * randDist);
        } else if(predator != null) {
            // Run specifically away
            try {
                targetPosition = new Vector3(random.Next(-5000.0f, 5000.0f),
                                     1.0f, random.Next(-5000.0f, 5000.0f)) *
        ...
        if(SpeciesAggression > SpeciesFear &&
            RollReverseCheck(SpeciesFocus, 400.0f, random)) {
            if(microbe.Hitpoints > 0 && microbe.AgentVacuoleCount > 0 &&
                (microbe.Translation - targetPosition).LengthSquared() <=
                    SpeciesFocus * 10.0f) {
                if(microbe.Compounds.GetCompoundAmount(oxytoxy) >=
...
private
    void
        EvaluateEnvironment(Random random)
    {
        if(RollCheck(SpeciesOpportunism, 500.0f, random)) {
            ...
            if(prey != null && predator != null) {
                if(random.Next(0.0f, SpeciesAggression) >
                    ...
                    lifeState = LifeState.PREDATING_STATE;
                } else if(RollCheck(SpeciesFocus, 500.0f, random) &&
                          random.Next(0, 10) <= 2) {
                ...
                if(RollCheck(SpeciesFocus, 500.0f, random) &&
                    random.Next(0, 10) <= 5) {
                    lifeState = LifeState.GATHERING_STATE;
                }
...
private
    void
        DoRunAndTumble(Random random)
    {
        // Run and tumble
        // A biased random walk, they turn more if they are picking up less
        // compounds.
        ...
        // Angle should only change if you havent picked up compounds || picked
        // up less compounds
        ...
        if(compoundDifference < Constants.AI_COMPOUND_BIAS &&
            random.Next(0, 10) < 9) {
            randAngle = SelectRandomTargetPosition(random);
        }
        ...
        // Turn more if not in concentration gradient basically (step is .4 if
        // really no food, .3 if less food, .1 if in food)
        previousAngle = randAngle;
...
private
    float
        SelectRandomTargetPosition(Random random)
    {
    ...
    /// <summary>
    ///   This makes the microbe to do some random movement, used by the AI when
    ///   nothing else should be done
    /// </summary>
private
    void
        SetRandomTargetAndSpeed(Random random)
    {
        // Set a random nearby look at location
        microbe.LookAtPoint =
            microbe.Translation +
            new Vector3(random.Next(-200, 201), 0, random.Next(-200, 201));

        // And random movement speed
        microbe.MovementDirection =
            new Vector3(0, 0, (float)(-1 * random.NextDouble()));
    }
}
