    ...
    ProcessSystem::Run(GameWorld& world)
{
    ...
    for(auto& value : CachedComponents.GetIndex()) {
        ...
        if(processor->process_capacities.size() > 0)
		{
            for(const auto& process : processor->process_capacities) {
                ...
                // This should not do anything if the cell has no room to hold
                // the new compounds and it shouldn't just keep draining
                // compounds if you lack the stuff you need to Do your processes
                ...
                if(processCapacity != 0.0f) {
                    ...
                    // Loop through to make sure you can follow through with
                    // your whole process so nothing gets wasted as that would
                    // be frusterating, its two more for loops, yes but it
                    // should only really be looping at max two or three times
                    // anyway. also make sure you wont run out of space when you
                    // do add the compounds. Input
                    for(const auto& input :
                        SimulationParameters::bioProcessRegistry
                            .getTypeData(processId)
                            .inputs) {
                        CompoundId inputId = input.first;
                        // Set price of used compounds to 1, we dont want to
                        // purge those
                    ...
                    // Output
                    // Dont loop if you dont need to so check if canDoProcess
                    ...
                    // Even if you cannot do the process, you still need to know
                    // the price I want to keep this code as simplistic as
                    // possible so we can comprehend it, so i might just add a
                    // new method specifically for calculating prices and call
                    // it as this seems messy.
                    else {
                        for(const auto& output :
                            SimulationParameters::bioProcessRegistry
                                .getTypeData(processId)
                    ...
                    // ingrediants, and if something weird happens and you
                    // suddenly lose your capability, just remove what you can
                    // and get out and next time you will be unable
