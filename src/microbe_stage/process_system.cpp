...
{
    return 2 * sigmoid(processCapacity * PROCESS_CAPACITY_DEMAND_MULTIPLIER) -
           1.0;
}


double
    ProcessSystem::_calculatePrice(double oldPrice,
        double supply,
        double demand)
{
    ...
    //(void)oldPrice;
    return sqrt(demand / (supply + 1)) * COMPOUND_PRICE_MOMENTUM +
           oldPrice * (1.0 - COMPOUND_PRICE_MOMENTUM);
}

std::map<double, CompoundId>
    ProcessSystem::_getBreakEvenPointMap(BioProcessId processId,
...
double
    ProcessSystem::_spaceSofteningFunction(double availableSpace,
        double requiredSpace)
{
    ...
    // return 1.0 / (1 + requiredSpace / std::max(availableSpace,
    // MIN_AVAILABLE_SPACE));
}

double
    ProcessSystem::_getOptimalProcessRate(BioProcessId processId,
        CompoundBagComponent& bag,
        ...
        double availableSpace)
{
    ...
    for(const auto& input :
        ...
        double inputVolume =
            SimulationParameters::compoundRegistry.getTypeData(inputId).volume;
        ...
        if(considersSpaceLimitations) {
            ...
            inputPriceIncrement += inputNeeded *
                                   compoundData.priceReductionPerUnit *
                                   spacePriceDecrement;
            baseInputPrice +=
                inputNeeded * compoundData.price * spacePriceDecrement;
        }

        else {
            inputPriceIncrement +=
                inputNeeded * compoundData.priceReductionPerUnit;
            baseInputPrice += inputNeeded * compoundData.price;
        }
    }

    // Finding the rate at which the costs equal the benefits.
    // The benefit curve is piecewise lineal and continuous, and the breaking
    // points are the break-even points of the output compounds. So first we
    // have to order said break-even points.
    std::map<double, CompoundId> outputBreakEvenPoints =
        ProcessSystem::_getBreakEvenPointMap(processId, bag);
    ...
    for(const auto& output :
        ...
        CompoundData& compoundData = bag.compounds[outputId];
        double outputVolume =
            SimulationParameters::compoundRegistry.getTypeData(outputId).volume;

        if(considersSpaceLimitations) {
            double spacePriceDecrement = ProcessSystem::_spaceSofteningFunction(
                availableSpace, outputGenerated * outputVolume);
            baseOutputPrice +=
                compoundData.price * outputGenerated * spacePriceDecrement;
            outputPriceDecrement += compoundData.priceReductionPerUnit *
                                    outputGenerated * spacePriceDecrement;
        }

        else {
            baseOutputPrice += compoundData.price * outputGenerated;
            outputPriceDecrement +=
                compoundData.priceReductionPerUnit * outputGenerated;
        }
    }
    ...
    for(const auto& breakingPoint : outputBreakEvenPoints) {
        ...
        for(const auto& output :
            ...
            CompoundData& compoundData = bag.compounds[outputId];
            double outputVolume =
                SimulationParameters::compoundRegistry.getTypeData(outputId)
                    .volume;

            // The prices are never below 0.
            if(compoundData.breakEvenPoint > breakEvenPoint) {
                if(considersSpaceLimitations) {
                    double spacePriceDecrement =
                        ProcessSystem::_spaceSofteningFunction(
                            availableSpace, outputGenerated * outputVolume);
                    baseOutputPrice_l += compoundData.price * outputGenerated *
                                         spacePriceDecrement;
                    outputPriceDecrement_l +=
                        compoundData.priceReductionPerUnit * outputGenerated *
                        spacePriceDecrement;
                }
        ...
        double revenue =
            baseOutputPrice_l - breakEvenPoint * outputPriceDecrement_l;
    ...
    if(outputPriceDecrement + inputPriceIncrement > 0)
        desiredRate = (baseOutputPrice - baseInputPrice) /
                      (outputPriceDecrement + inputPriceIncrement);
    else
        desiredRate = 0.0;
    if(desiredRate <= 0.0)
        return 0.0;
    ...
    ProcessSystem::Run(GameWorld& world)
{
    ...
    for(auto& value : CachedComponents.GetIndex()) {
        ...
        // Calculating the storage space available. The storage space capacity
        // is increased
        double storageSpaceAvailable =
            std::max(bag.storageSpace - bag.storageSpaceOccupied, 0.0);
        ...
        for(const auto& compound : bag.compounds) {
            ...
            compoundData.uninflatedPrice = ProcessSystem::_calculatePrice(
                oldPrice, compoundData.amount, compoundData.demand);

            if(compoundData.demand > 0 &&
                compoundData.uninflatedPrice <= MIN_POSITIVE_COMPOUND_PRICE) {
                compoundData.uninflatedPrice = MIN_POSITIVE_COMPOUND_PRICE;
            }

            // Setting the prices to 0 if they're below
            ...
            // Calculating how much the price would fall if we had one more
            // unit, To make predictions with the demand.
            else {
                double reducedPrice = ProcessSystem::_calculatePrice(
                    oldPrice, compoundData.amount + 1, compoundData.demand);

                compoundData.priceReductionPerUnit =
                    compoundData.uninflatedPrice - reducedPrice;
            }

            // Inflating the price if the compound is useful outside of this
            // system.
            compoundData.price = compoundData.uninflatedPrice;
            if(SimulationParameters::compoundRegistry.getTypeData(compoundId)
                    .isUseful) {
                compoundData.price +=
                    (IMPORTANT_COMPOUND_BIAS + bag.storageSpace) /
                    (compoundData.amount + 1);
                double reducedPrice =
                    (IMPORTANT_COMPOUND_BIAS + bag.storageSpace) /
                    (compoundData.amount + 2);
                compoundData.priceReductionPerUnit +=
                    compoundData.price - reducedPrice;
            }
            ...
            if(compoundData.price <= 0.0)
                ...
                compoundData.breakEvenPoint =
                    compoundData.price / compoundData.priceReductionPerUnit;
        ...
        for(const auto& process : processor.process_capacities) {
            ...
            // Calculating the desired rate, with some liberal use of
            // linearization.

            // Calculating the optimal process rate without considering the
            // storage space.
            double desiredRate = ProcessSystem::_getOptimalProcessRate(
                processId, bag, false, storageSpaceAvailable);

            // Calculating the optimal process rate considering the storage
            // space.
            double desiredRateWithSpace = ProcessSystem::_getOptimalProcessRate(
                processId, bag, true, storageSpaceAvailable);

            desiredRateWithSpace = std::min(desiredRateWithSpace, desiredRate);
            if(desiredRate > 0.0) {
                double rate = std::min(
                    processCapacity * logicTime / 1000, processLimitCapacity);
                rate = std::min(rate, desiredRateWithSpace);

                // Running the process at the specified rate, transforming the
                // inputs...
                for(const auto& input : SimulationParameters::bioProcessRegistry
                    ...
                    bag.compounds[inputId].demand +=
                        desiredRate * inputNeeded *
                        ProcessSystem::_demandSofteningFunction(
                            processCapacity * inputNeeded);
                }
