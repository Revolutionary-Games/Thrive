...
#pragma warning restore CA2213
    ...
    {
        // Get required components from the scene tree
    ...
    private void UpdateRippleEffect(float delta)
    {
        ...
        currentAlpha = Mathf.Lerp(currentAlpha, targetAlpha, delta * (currentAlpha < targetAlpha ? fadeInSpeed : fadeOutSpeed));
    ...
    private void UpdateDetailLevelSubdivisions()
    {
        ...
        float t = Mathf.Clamp(scaledDistance / 120.0f, 0.0f, 1.0f);
        t = Mathf.Pow(t, 1.5f); // Non-linear curve
    ...
    private void UpdateLocalPositions()
    {
        ...
        for (int i = 0; i < count; i += step)
        {
            ...
            // Update both arrays ensuring array bounds
            int targetIndex = i / step;
            if (targetIndex < pastPositions.Length && targetIndex < godotPastPositions.Count)
            {
        ...
        // Fill remaining positions with zeros for lower detail sampling
        if (step > 1)
        {
            for (int i = count / step + 1; i < count && i < pastPositions.Length && i < godotPastPositions.Count; ++i)
            {
                pastPositions[i] = Vector2.Zero;
