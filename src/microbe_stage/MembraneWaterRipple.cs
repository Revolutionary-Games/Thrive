...
#pragma warning restore CA2213
    ...
    private void ProcessFadeState(float delta)
    {
        ...
        if ((currentFadeState == FadeState.FadingIn || currentFadeState == FadeState.FadingOut ||
             currentFadeState == FadeState.Forming) && stateTime > MAX_TRANSITION_TIME)
        {
            // Force completion of transition if stuck
            if (currentFadeState == FadeState.Forming || currentFadeState == FadeState.FadingIn)
            {
                fadeProgress = 1.0f;
    ...
    private void InitializeLodMeshes()
    {
        ...
        if (LodMeshes[0] == null)
        {
            for (int i = 0; i < 4; ++i)
            {
                ...
                LodMeshes[i].Size = new Vector2(baseSize, baseSize);

                // Set appropriate subdivision based on LOD level
                switch ((LodLevel)i)
                {
                    case LodLevel.VeryLow:
                        LodMeshes[i].SubdivideWidth = VERY_LOW_LOD_SUBDIVISION;
                        LodMeshes[i].SubdivideDepth = VERY_LOW_LOD_SUBDIVISION;
                        break;
                    case LodLevel.Low:
                        LodMeshes[i].SubdivideWidth = LOW_LOD_SUBDIVISION;
                        LodMeshes[i].SubdivideDepth = LOW_LOD_SUBDIVISION;
                        break;
                    case LodLevel.Medium:
                        LodMeshes[i].SubdivideWidth = MEDIUM_LOD_SUBDIVISION;
                        LodMeshes[i].SubdivideDepth = MEDIUM_LOD_SUBDIVISION;
                        break;
                    case LodLevel.High:
                        LodMeshes[i].SubdivideWidth = HIGH_LOD_SUBDIVISION;
                        LodMeshes[i].SubdivideDepth = HIGH_LOD_SUBDIVISION;
                        break;
                }
            }
        }
    ...
    private void UpdateMovementParameters(float delta)
    {
        ...
        if (significantMovement)
        {
            ...
            if (currentFadeState == FadeState.Minimal || currentFadeState == FadeState.FadingOut)
            {
                TransitionToState(FadeState.FadingIn);
            }
