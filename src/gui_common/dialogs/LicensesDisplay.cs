...
#pragma warning restore CA2213
    ...
    public static string LoadSteamLicenseFile()
    {
        try
        {
            return LicenseText.LoadSteamLicenseFile(true, LoadFile);
        }
        catch (Exception e)
        {
            GD.PrintErr("Failed to load Steam license file due to exception: ", e);
    ...
    public override void _Ready()
    {
        ...
        [
            (string.Empty,
                isSteamVersion ? LoadSteamLicenseFile : () => LicenseText.LoadNormalLicenseText(true, LoadFile)),
    ...
    {
        // Keep the licence texts only loaded when this is visible
        if (IsVisibleInTree())
        {
            if (licensesLoaded)
                return;
    ...
    internal static string LoadFile(string file)
    {
