...
float checker_pattern(vec2 newUV) {
    newUV = floor(newUV);
    float filter = 1.0 - mod(newUV.y + newUV.x / 2., coumpound_count / 2.);
    return floor(clamp(filter, 0., 1.));
}

//returns icon i at coordinate uv
vec4 textureByIndex(int i, vec2 newUV) {
    switch (i) {
        case 0:
            return texture(icon1, newUV);
        case 1:
            return texture(icon2, newUV);
        case 2:
            return texture(icon3, newUV);
        case 3:
            return texture(icon4, newUV);
    }
}


vec4 get_pattern(int icon_index, vec2 uv) {
    float pattern_size = 1.0 / scale;
    vec2 tile_size = 1.0/vec2(pattern_size);

    vec4 compound_texture = vec4(0.);
    vec2 overlap_offset = tile_size * vec2(float(icon_index + compound_count_offset),0.);
    
    vec2 newUV = uv + overlap_offset + UVOffset;
    newUV *= pattern_size;

    float checker = checker_pattern(newUV);
    // if (checker == 1.0)
    if (checker > 0.9) compound_texture = textureByIndex(icon_index, newUV);

    return compound_texture;
}
...
void fragment(){
    ...
    if (enable_texture) {
        vec4 tex_colour1 = get_pattern(0, UV);
        vec4 tex_colour2 = get_pattern(1, UV);
        vec4 tex_colour3 = get_pattern(2, UV);
        vec4 tex_colour4 = get_pattern(3, UV);
        
        swap_colour1 = mix(tex_colour1, colour1, blend_factor);
        swap_colour2 = mix(tex_colour2, colour2, blend_factor);
        swap_colour3 = mix(tex_colour3, colour3, blend_factor);
        swap_colour4 = mix(tex_colour4, colour4, blend_factor);
    }
